		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					const parts: string[] = [];
					parts.push(dim(ctx.model?.name || ctx.model?.id || "no-model"));

					const cu = ctx.getContextUsage();
					if (cu && cu.contextWindow) {
						const pctVal = cu.percent;
						const pct = pctVal === null ? "?" : pctVal.toFixed(1);
						const ctxStr = `${pct}% of ${fmt(cu.contextWindow)}`;
						let colored: string;
						if (pctVal !== null && pctVal > 90) colored = theme.fg("error", ctxStr);
						else colored = dim(ctxStr);
						parts.push(colored);
					}
					...
					if (tok.length) parts.push(dim(tok.join(" ")));

					if (cost > 0) parts.push(dim(`¥${cost.toFixed(2)}`));

					if (ctx.model?.reasoning) {
						if (thinkingLevel && thinkingLevel !== "off") {
							parts.push(dim(`${thinkingLevel} thinking`));
						} else {
							parts.push(dim("fast"));
						}
					}
